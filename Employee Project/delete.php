...
{    
	$ename=$_POST['ename'];
       ...
       $sql="DELETE FROM employee_record WHERE id='$search_query_parameter' ";
 
       $execute=$connect->query($sql);
      
       if($execute)
       {
       	echo "<script> window.open('viewdb.php?id=record deleted sucessfully','_self')</script>";//this statement brings us back to prevoius window(viewdb wali) ,it using js
       }
